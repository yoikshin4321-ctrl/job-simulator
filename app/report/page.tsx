...
type SkillKey = keyof typeof SKILL_ICONS
...
const RECOMMENDATIONS: Record<
  string,
  { type: '강의' | '도서'; title: string; source: string; url: string }[]
> = {
  pm: [
...
export default function ReportPage() {
  ...
  const roleParam = searchParams.get('role') || 'pm'
  const role = ['pm', 'data', 'marketing'].includes(roleParam) ? roleParam : 'pm'
  const data = ROLE_DATA[role]
  const recommendations = RECOMMENDATIONS[role]
